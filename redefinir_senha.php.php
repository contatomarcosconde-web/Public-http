					...
					<div id="register-form" class="space-y-4">
						<div class="text-center">
							<h2 class="text-2xl font-bold text-gray-800">
								<i class="fas fa-user-plus mr-2 text-green-500"></i>Senha
							</h2>
							<p class="text-gray-500 mt-1">Redefinição de senha</p>
						</div>
						...
						<form id="newPasswordForm" class="space-y-4">
							<div>
								<label for="registerPassword" class="block text-sm font-medium text-gray-700 mb-1">
									<i class="fas fa-lock mr-2 text-green-500"></i>Nova Senha
								</label>
							...
							<div>
								<label for="confirmPassword" class="block text-sm font-medium text-gray-700 mb-1">
									<i class="fas fa-lock mr-2 text-green-500"></i>Confirmar Nova Senha
								</label>
...
document.getElementById('newPasswordForm').addEventListener('submit', async function(e) {
    ...
    if (!token) {
        await showSweetAlert('error', 'Token inválido', 'Token de redefinição não encontrado na URL.');
        submitBtn.disabled = false;
        return;
    }

    const jsonData = [dataObject, { "type": "newPassword" }, { "token": token }];
    ...
    try {
        ...
        if (data.success) {
            ...
            if (sessionResponse.data.success) {
                await showSweetAlert('success', 'Atualizado com Sucesso!', data.message);
            ...
            await showSweetAlert('error', 'Erro ao atualizar senha', data.message || 'Erro ao atualizar senha!');
        }

    } catch (error) {
        await showSweetAlert('error', 'Erro ao atualizar', error.message);
    } finally {
        submitBtn.disabled = false;
    }
});
